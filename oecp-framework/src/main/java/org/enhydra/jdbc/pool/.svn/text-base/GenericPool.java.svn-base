...
public class GenericPool {
	...
	public synchronized void checkIn(Object o) {
		...
		for (Enumeration enums = locked.keys();
			enums.hasMoreElements();
			) { // for each object of
			GenerationObject obj = (GenerationObject) enums.nextElement();
			// the locked pool
		...
		if (count > maxSize) {
			...
			Enumeration enums = unlocked.keys(); // get the unlocked pool
			for (int i = maxSize;
				i < count;
				i++) { // try to remove object from this pool
				if (getUnlockedObjectCount() > 0) {
					GenerationObject obj =
						(GenerationObject) enums.nextElement();
					removeUnlockedObject(obj);
				}
			}
	...
	public synchronized void setMaxSize(int max) throws Exception {
		if (max < 0)
			...
			if (count > max) { // if pool has more than max element
				...
				Enumeration enums = unlocked.keys();
	...
	protected void cleanUp() {
		...
		long now = System.currentTimeMillis(); // current time
		synchronized (this) {
		        for (Enumeration enums = unlocked.keys();
		        	enums.hasMoreElements();
		        	) { // for each object of the
		        	GenerationObject o = (GenerationObject) enums.nextElement();
	        		// unlocked pool
