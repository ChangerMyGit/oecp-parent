...
public class QueryObject implements Serializable {
	...
	protected void buildQuery() {
		...
		for (int i = 0; i < size; i++) {
			...
			if("like".equalsIgnoreCase((condition.getOperator().toString()))){		//对Like的处理		
				paramList.add("%"+condition.getValue()+"%");
			}else if("not like".equalsIgnoreCase((condition.getOperator().toString()))){		//对not Like的处理		
				paramList.add("%"+condition.getValue()+"%");
			}else if("like ".equalsIgnoreCase((condition.getOperator().toString()))){ //左边相似
				paramList.add(condition.getValue()+"%");
			}else if(" like".equalsIgnoreCase((condition.getOperator().toString()))){ // 右边相似
				paramList.add("%"+condition.getValue());
			}else{
				paramList.add(condition.getValue());
			}
		}
		queryParams = paramList.toArray();
