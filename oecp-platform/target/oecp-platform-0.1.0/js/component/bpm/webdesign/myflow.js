(function(b) {
	...
	a.rect = function(p, m) {
		...
		n = C.text(E.attr.x + E.img.width + (E.attr.width - E.img.width) / 2,
				E.attr.y + a.config.lineHeight / 2, E.name.text).hide()
				.attr(E.name);
		f = C.text(
			...
			switch (M) {
				case "from" :
					r = z.rect(H.x - B.attr.fromDot.width / 2,
							H.y - B.attr.fromDot.height / 2,
							B.attr.fromDot.width, B.attr.fromDot.height)
							.attr(B.attr.fromDot);
					break;
				case "big" :
					r = z.rect(H.x - B.attr.bigDot.width / 2,
							H.y - B.attr.bigDot.height / 2,
							B.attr.bigDot.width, B.attr.bigDot.height)
							.attr(B.attr.bigDot);
					break;
				case "small" :
					r = z.rect(H.x - B.attr.smallDot.width / 2,
							H.y - B.attr.smallDot.height / 2,
							B.attr.smallDot.width, B.attr.smallDot.height)
							.attr(B.attr.smallDot);
					break;
				case "to" :
				...
				switch (M) {
					...
					case "small" :
						if (o && O && !a.util.isLine(o.pos(), {
							...
							var P = new p("small", a.util.center(o.pos(), N),
									o, o.right());
							o.right(P);
							o = P;
	...
	a.props = function(h, f) {
		...
		var d = function(n, m, o) {
			...
			c.show();
			for (var l in m) {
				if(l!='remove'&&l!='indexOf'){
					e.append("<tr><th  nowrap>" + m[l].label + '</th><td><div id="p' + l
	...
	a.init = function(x, r) {
		var v = b(window).width(), e = b(window).height(), y = Raphael(x, v
						* 1.5, e * 1.5), q = {}, g = {};
		...
		b(y).bind("addrect", function(j, c, k) {
			var i = new a.rect(b.extend(true, {}, a.config.tools.states[c], k),
					y);
			q[i.getId()] = i
		});
