...
Ext.Drawing.svgRenderer=function(config){
		...
		else
		{
			switch(config.type)
			{
				...
				case "circle":
					...
					s.setX=function(x)
					{
						s.set({"cx":x+s.getWidth()/2});
					};
					s.setY=function(y)
					{
						s.set({"cy":y+s.getHeight()/2});
					};
					s.setXY=function(x,y)
					{
						s.set({
			...
			{
				s.set({fill:this.createFill(config.fill)});
			}
			else
				s.set({fill:"none"});
			if(config.stroke)
	...
	this.init=function(e,s)
	{
		...
		el.setAttribute("width",config.width+"px");
		el.setAttribute("height",config.height+"px");
		e.insertFirst(el);
	...
	this.clear=function(){
		//new Ext.Element(this.getRoot()).update("");
		while (this.getRoot().firstChild) {
			this.getRoot().removeChild(this.getRoot().firstChild);
		}
		var def=document.createElementNS("http://www.w3.org/2000/svg","defs");
		this.getRoot().appendChild(def);
		this.defs=def;
	}
...
Ext.Drawing.vmlRenderer=function(config){
	this.createShape=function(config){
		var d,s;
		...
		else
		{
			switch(config.type)
			{
				...
				case "text":
					...
					break;
				case "path":
					d=this.doc.createElement("v:shape");
					t=this.doc.createElement("v:path");
	...
	this.createFill=function(config,o)
	{
		...
		else
		{
			...
			{
				if(config.type == "color" || (!Ext.isEmpty(config.color) && !((config.type == "linear")||(config.type == "radial")))){
...
Ext.Drawing.Surface=function(config){
	if(!Ext.isEmpty(config))
	{
		...
		{
			case "svg":
				this.render=new Ext.Drawing.svgRenderer(config);
				break;
			case "vml":
				this.render=new Ext.Drawing.vmlRenderer(config);
				break;
			default:
				if(Ext.isIE)
					this.render=new Ext.Drawing.vmlRenderer(config);
		...
		// 2011-12-16 slx 删除下面4行，防止外层panel缩小。
	//	if(!Ext.isEmpty(config.width))
	//		config.El.setWidth(config.width);
	//	if(!Ext.isEmpty(config.height))
	//		config.El.setWidth(config.height);
	}
	else
	{
		if(Ext.isIE)
