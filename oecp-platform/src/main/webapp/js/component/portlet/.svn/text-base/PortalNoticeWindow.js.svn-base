 ...
 */
OECP.portlet.PortalNoticeWindow = Ext.extend(OECP.ui.CommonWindow,{
	width : 600,
	height : 520,
	title : '公告',
	/**
	...
	initComponent : function(){
		...
		OECP.portlet.PortalNoticeWindow.superclass.initComponent.call(this);
	},
	/**
	 * 初始化窗口里面的按钮
	 * @param {} me
	...
	initWindowBtns : function(me){
		var saveBtn = new OECP.ui.Button({
				...
				iconCls : 'btn-save',
				handler : function(){
					var data = me.PortalNoticePanel.getFormValues();
					if(!data[0])
						return;
					//----------lyc
...
//						return;
	...
	initContent : function(me){
		if(!Ext.isDefined(me.noticePanel)){
			me.PortalNoticePanel = new OECP.portlet.PortalNoticePanel({isView:me.isView});
		}
	...
	putValues : function(me){
		Ext.Ajax.request({
			...
			success : function(request) {
				var json = Ext.decode(request.responseText);
				if (json.success) {
					var rs = me.PortalNoticePanel.NoticeFormPanel.reader.readRecords(json);
					var _data = rs.records && rs.records[0] ? rs.records[0].data : null;
					me.PortalNoticePanel.NoticeFormPanel.getForm().setValues(_data);
					//给任务组combo赋值
