 ...
 */
Ext.ns("OECP.bpmperson.undotask");
 ...
 * 
 * @author yangtao
 * @class OECP.bpmperson.PersonalTaskView
 * @extends Ext.Panel
 */
OECP.bpmperson.PersonalTaskView =  Ext.extend(Ext.Panel, {
	/**
	 * id
	 */
	id : 'PersonalTaskViewForLogin',
	/**
	 * 标题
	 */
	//title : "流程待办任务列表",
		...
		Ext.Ajax.request({
				...
				success:function(request){
					...
					var fromPanel = new Ext.Panel({
								...
								frame : true,
								autoScroll : true,
								html:'<iframe frameborder="no" border="0" height="350" width="740" src="'+bill_URL+'"></iframe>'
					});
					...
					var nextTaskUserCombo = new Ext.ux.form.LovCombo({
						...
						store:{
							xtype:'jsonstore',url:__ctxPath+'/bpm/task/queryUserList.do',
						...
						var bpmAuditFormForUndoTaskForNoCounterSign = new Ext.FormPanel({
							        ...
							        buttons: [{
							            ...
							            handler : function(){
							            	...
							            	var actionUtil = new BpmPersonActionUtil();
	 ...
	 * @param activityName
	 */
	reasign : function(mainGrid,taskId,assignOrgId,activityName){
		...
		var win = new Ext.Window({
			...
			buttons : [{
				...
				handler : function() {
					...
					var actionUtil = new BpmPersonActionUtil();
					actionUtil.reassignVirProActivity({taskId:taskId,assignOrgId:assignOrgId,ids:ids,assignFlag:assignFlag},win);
				}
			}, {
				xtype : 'button',
				handler : function() {
	...
	initComponent : function() {
		 ...
		 * 初始化父类
		 */
		OECP.bpmperson.PersonalTaskView.superclass.initComponent.call(this);
		...
		this.store = new Ext.data.JsonStore({
			url :__fullPath+"bpm/person/task/undoTask.do",
			baseParams : {
						billInfo : "",
						beginTime : "",
						endTime : "",
			...
			root : "result",
			remoteSort : true,
			fields : ["id","billInfo","billKey","taskId","taskName","assignee","executionId","formResourceName","deployId","processName","assignOrgId","createTime","counterSignRuleId","counterSignRuleName","operate","nextTask","nextTaskUser","editBill"],
			totalProperty : 'totalCounts'
		});
		...
		this.cm = new Ext.grid.ColumnModel([{
            ...
            items: [{
                ...
                handler: function(grid, rowIndex, colIndex) {
                    ...
                    scope.reasign(grid,taskId,assignOrgId,taskName);
                }
             }]
         }]);
