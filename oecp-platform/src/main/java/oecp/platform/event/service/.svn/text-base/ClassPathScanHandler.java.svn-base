...
public class ClassPathScanHandler {
    ...
    private void doScanPackageClassesByFile(Set<Class<?>> classes, String packageName, String packagePath, boolean recursive) {
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                ...
                try {
                	Class<?> cls = findListener(packageName + '.' + className);
                	if(cls != null){
                		classes.add(cls);
                	}
                } catch (ClassNotFoundException e) {
     ...
     * 判断是否是监听器
     * @author wangliang
     * @date 2012-2-8下午3:06:14
     * @param clsName
     * @return
     * @throws ClassNotFoundException
     */
    private Class<?> findListener(String clsName) throws ClassNotFoundException{
    	Class<?> cls = Thread.currentThread().getContextClassLoader().loadClass(clsName);
    	Annotation l =cls.getAnnotation(Listener.class);
    	if(l != null){
    		return cls;
    	}
    	return null;
	...
	public static void main(String[] args) throws SecurityException, NoSuchMethodException, IllegalArgumentException, IllegalAccessException, InvocationTargetException, InstantiationException {
		...
		Set<Class<?>> calssList = handler.getPackageAllClasses("cn", true);
		for (Class<?> cla : calssList) {
			System.out.println(cla.getName());
			 Method method = cla.getMethod("onTest", String.class);
